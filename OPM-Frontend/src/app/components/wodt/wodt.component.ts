...
export class WODTComponent implements OnInit {
  ...
  constructor(
    ...
    this.id = this.route.snapshot.paramMap.get("id"); // if add the id wose for the client else the id for worek order 
    ...
    if (this.typeOpertion == 'updateWorekOrder') {
      ...
      this.backendService
        ...
        .subscribe(
          new Observer(
            this.router,// just un class dans angular
            `workOrderDetail/${this.id}`,//target : lin eli machilou
            false,//relode
            true,//swwet alert
            this.sharedService,//obligtour si ona reload
          ).OBSERVER_POST()
